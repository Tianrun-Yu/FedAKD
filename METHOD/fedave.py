...
def mask_top_k(vec, fraction):
    """
    对 1D 向量 vec 保留 abs最大 fraction比例
    fraction in (0,1]
    """
    if fraction >= 1.0:
        return vec
...
def fedave_training(base_dir, lr, device):
    ...
    for round_idx in range(NUM_GLOBAL_ROUNDS):
        ...
        for cid in range(NUM_CLIENTS):
            ...
            data_sizes.append(len(train_loaders[cid].dataset))  # 简单地用样本数

            new_models[cid]= local_model.state_dict() # 本地新模型
        ...
        # 5) 服务器下发“奖励梯度” => 每个客户端
        #    fraction_i = DOWNLOAD_FRACTION_BASE + tanh(BETA*r_i)* (1-DOWNLOAD_FRACTION_BASE)
...
def load_standalone_accuracies(dataset_name, dist_name, num_epochs=NUM_GLOBAL_ROUNDS):
    """
    从 STANDALONE_DIR 下的 "{dataset_name}_{dist_name}.txt" 文件中读取 10 行准确率。
    文件格式示例:
      Client 0: 0.5560
      Client 1: 0.5400
      ...
    ...
    若文件不存在，则返回全 0 矩阵。
    读到的 10 个客户端准确率会复制为 (num_epochs, 10)，便于多轮的相关系数计算。
    """
    ...
    acc_array = np.zeros(NUM_CLIENTS, dtype=np.float32)
    ...
    # 复制成 (num_epochs, NUM_CLIENTS)
    expanded = np.tile(acc_array, (num_epochs,1))
    return expanded
