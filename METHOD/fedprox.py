...
# *独自训练* 文件所在目录 (请根据你实际的独自训练结果路径来调整)
...
def local_train_prox(model, global_params, train_loader, device, local_epochs, lr, mu):
    ...
    optimizer = optim.SGD(model.parameters(), lr=lr)  # 用SGD以便添加prox grad
...
def fedprox_training(base_dir, lr, device, mu):
    ...
    for round_idx in range(NUM_GLOBAL_ROUNDS):
        ...
        for cid in range(NUM_CLIENTS):
            ...
            # local train (FedProx)
            local_train_prox(local_model, global_params, train_loaders[cid],
                             device, LOCAL_EPOCHS, lr, mu)
...
def load_standalone_accuracies(dataset_name, dist_name, num_epochs=NUM_GLOBAL_ROUNDS):
    """
    从 STANDALONE_DIR 下读取名为 "{dataset_name}_{dist_name}.txt" 的文件,
    内容格式示例:
      Client 0: 0.5560
      Client 1: 0.5400
      ...
      Client 9: 0.4920
    解析后将 10 个客户端准确率复制扩展为 (num_epochs,10) 用于多轮对比。
    若文件不存在则返回全 0.
    """
    ...
    # 复制为多轮 (NUM_GLOBAL_ROUNDS, NUM_CLIENTS)
